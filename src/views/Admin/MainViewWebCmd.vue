...
const { proxy, data } : any= getCurrentInstance();
onBeforeMount(() => {
  ...
  if (AuthCode.length > 0) {
    ...
    const PromiseReq = proxy.$HelperAxios.post(proxy.$ConfigBiz.jsonFuncOption().API_Local.strURL + "/UC03UV040/AR03AT060", jsonReqParam).then((jsonResult:any) => {
      ...
      if (responseResult.SC120 === "SuccGoal") {
        ...
        // User Group
        let promiseUserGroup = proxy.$HelperUserAccount.jsonFuncUserGroup(2).then((jsonArrayUserGroupIn:any) => {
          ...
          if (URL_Return.length > 0) {
            ...
            window.location.href = "/AdminMainView";
          }
        });
