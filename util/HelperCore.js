...
class HelperCore {
  ...
  //针对 JSON 字符串过滤符号.
  static strFuncJsonFilter(strInput, boolActionId) {
    if (boolActionId) {
      return strInput.replaceAll(":", "PlaceholderColon").replaceAll(",", "PlaceholderComma").replaceAll('"', "PlaceholderQuote");
    } else {
      return strInput.replaceAll("PlaceholderColon", ":").replaceAll("PlaceholderComma", ",").replaceAll("PlaceholderQuote", '"');
    }
  }
  ...
  static domFuncAddCssFile(strArrayFilePath, FollowUpFunc) {
    let fileQuantity = 0;
    let strArrayExistChk = [];
    const elemExistChk = document.querySelectorAll("link");
    if (elemExistChk !== null && elemExistChk.length > 0) {
      elemExistChk.forEach(function (srcElemLoop) {
        strArrayExistChk.push(srcElemLoop.getAttribute("href"));
      });
    }
    for (let filePathLoop of strArrayFilePath) {
      if (!strArrayExistChk.includes(filePathLoop)) {
        let elemConfig = document.createElement("link");
        elemConfig.href = filePathLoop;
        elemConfig.rel = "stylesheet";
        elemConfig.type = "text/css";
        document.head.append(elemConfig);
        elemConfig.onload = elemConfig.onreadystatechange = function () {
          if (!this.readyState || this.readyState == "loaded" || this.readyState == "complete") {
            fileQuantity++;
            if (fileQuantity === strArrayFilePath.length) {
              FollowUpFunc();
            }
          }
          elemConfig.onload = elemConfig.onreadystatechange = null;
        };
      }
    }
  }
...
class HelperWebComm {
  static reqFuncAccessToken(URL_TargetIn, JsonReqParam, strMethodIn) {
    const accessToken = window.localStorage.getItem("AccessToken");
    if (accessToken === null) {
      return Promise.resolve(["NullAT060", ""]);
    } else {
      return this.reqFuncUniv(URL_TargetIn, { authorization: accessToken }, JsonReqParam, strMethodIn);
    }
  }
  static reqFuncUniv(URL_TargetIn, jsonReqHeaderIn, objReqParamBodyIn, strMethodIn) {
    ...
    const promiseRespResult = fetch(URL_TargetIn, InitVector)
      ...
      .then((StrRespResult) => {
        let strArrayRespResult = ["FailWR010", ""];
        if (StrRespResult !== "FailWR010") {
          strArrayRespResult[0] = "SuccWR010";
...
class HelperUserAccount {
  static handlerLogin() {
    window.location.href = ConfigBiz.jsonFuncOption().API_OAuth.strURL + "/LV06UA010/UV04LP010?NumAI020=" + ConfigBiz.jsonFuncOption().AppInfo.NumAppId + "&LR01AT060=" + encodeURIComponent(ConfigBiz.URL_LocalFunc()) + "&FP03SC160=" + encodeURIComponent("/MainViewWebCmd");
  }
  static jsonFuncUserGroup(numUpdateModeIn) {
    ...
    if (numUpdateModeIn === 3 || (numUpdateModeIn === 2 && jsonStrUserGroup === null)) {
      const promiseRespResult = HelperWebComm.reqFuncAccessToken(ConfigBiz.jsonFuncOption().API_Local.strURL + "/UC03AU010/QD01UG010", { SL060: "en" }, "post").then((strArrayRespResult) => {
        if (strArrayRespResult[0] === "NullAT060") {
          alert("Please login again.");
          HelperUserAccount.handlerLogin();
        }
        else if (strArrayRespResult[0] === "SuccWR010") {
          const jsonRespResult = JSON.parse(strArrayRespResult[1]);
          if (jsonRespResult.SC120 === "SuccGoal") {
            sessionStorage.setItem("jsonArrayUserGroup", JSON.stringify(jsonRespResult.Payload.JA01UG010));
            return jsonRespResult.Payload.JA01UG010;
          } else {
            return null;
          }
        } else {
  ...
  static jsonFuncCurrentUserGroup() {
    return this.jsonFuncUserGroup(2).then((jsonArrayUserGroupIn) => {
      ...
      if (jsonArrayUserGroupIn.length > 0) {
        ...
        const strCurrentUserGroupNum = sessionStorage.getItem('NumCG040');
...
class DataModelDataGroup {
  static jsonFuncSystemMgt(numUpdateModeIn, numCurrentUserIn) {
    ...
    if (numUpdateModeIn === 3 || (numUpdateModeIn === 2 && (jsonDataGroupInner === null || !HelperCore.boolFuncJsonExistChk(jsonDataGroupInner, StrKeyM0)))) {
      let promiseDataGroupInner = HelperWebComm.reqFuncAccessToken(ConfigBiz.jsonFuncOption().API_Local.strURL + "/DA080/QD01DG010", { SL060: "en", CG040: numCurrentUserIn }, "get").then((strArrayRespResult) => {
